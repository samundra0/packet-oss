...
# Check Node.js — install v22 if missing or too old.
# Node 22+ is required because the current pnpm (11.x) uses the node:sqlite
# built-in module, which is unavailable on Node 20 and earlier.
NEED_NODE=false
if ! command -v node &>/dev/null; then
  ...
  if [[ "$NODE_VERSION" -lt 22 ]]; then
    warn "Node.js 22+ required (found v${NODE_VERSION}), upgrading..."
    NEED_NODE=true
  fi
fi

if $NEED_NODE; then
  run_with_progress "Installing Node.js 22" bash -c "export DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a && curl -fsSL https://deb.nodesource.com/setup_22.x | bash - && apt-get install -y -qq nodejs"
fi
